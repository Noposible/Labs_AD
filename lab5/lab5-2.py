...
init_filter_order   = 5     # Початковий порядок фільтра
...
#Фільтр IIR (за допомогою iirfilter)
def iir_lowpass_filter(data, cutoff, fs=100, order=5):
    nyq = 0.5 * fs
...
filtered_noise = iir_lowpass_filter(noise, init_cutoff)
...
l_filtered, = ax.plot(t, harmonic + filtered_noise,
                                linestyle='-',  color='forestgreen', label='Фільтрована гармоніка')
...
ax_order  = plt.axes([0.2, 0.08, 0.65, 0.03])
...
s_order  = Slider(ax_order,  'Порядок фільтра',     1, 10,     valinit=init_filter_order, valstep=1)
...
def update_harmonic(val):
    ...
    # якщо шум увімкнений зберігаємо старий noise тільки додаємо до нової гармоніки
...
def update_noise(val):
    ...
    filtered_noise = iir_lowpass_filter(noise, s_cutoff.val, order=int(s_order.val))
    # оновлюємо шумні лінії
    if check.get_status()[0]:
        l_noisy.set_ydata(harmonic + noise)
...
def reset(event):
    ...
    s_ncov.reset();   s_cutoff.reset(); s_order.reset()
...
s_order.on_changed(update_noise)
